...
function Octree:get_colliding(checkBounds, filter)
	return self.rootNode:get_colliding(checkBounds, filter)
end
...
function Octree:draw_bounds(cube, shader, position)
	self.rootNode:draw_bounds(cube, shader, position)
end

--- Draws the bounds of all objects in the tree visually for debugging.
function Octree:draw_objects(cube, shader, position, filter)
	self.rootNode:draw_objects(cube, shader, position, filter)
end
...
-- @return table Objects that intersect with the specified bounds
function OctreeNode:get_colliding(checkBounds, filter, results)
	local intersect_check = intersect.aabb_aabb

	filter = filter or function() return true, false end

	if not checkBounds.min then
		intersect_check = intersect.aabb_frustum
	end

	results = results or {}

	-- Are the input bounds at least partially in this node?
	if not intersect_check(self.bounds, checkBounds) then
		return results
	end

	-- Check against any objects in this node
	for _, object in ipairs(self.objects) do
		local pass, force = filter(object.data)
		if pass then
			local vis, dist = intersect_check(object.bounds, checkBounds) 
			if vis or force then
				if dist then
					object.distance_from_camera = dist
				end
	...
	-- Check children
	for _, child in ipairs(self.children) do
		results = child:get_colliding(checkBounds, filter, results)
	end
...
function OctreeNode:draw_bounds(cube, shader, position, depth)
	depth = depth or 0
	local tint = depth / 7 -- Will eventually get values > 1. Color rounds to 1 automatically

	if self.center:dist(position) < 150 then
		love.graphics.setColor(tint * 255, 0, (1 - tint) * 255)
		local m = mat4()
		m:translate(m, self.center)
		m:scale(m, vec3(self.adjLength, self.adjLength, self.adjLength))
	...
	for _, child in ipairs(self.children) do
		child:draw_bounds(cube, shader, position, depth + 1)
	end
...
--- Draws the bounds of all objects in the tree visually for debugging.
-- @param cube Cube model to draw
function OctreeNode:draw_objects(cube, shader, position, filter)
	...
	for _, object in ipairs(self.objects) do
		if filter and filter(object.data) or not filter then
			if object.bounds.center:dist(position) < 150 then
				local m = mat4()
				m:translate(m, object.bounds.center)
				m:scale(m, object.bounds.size)
	...
	for _, child in ipairs(self.children) do
		child:draw_objects(cube, shader, position, filter)
	end
