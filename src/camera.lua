...
local function new(options)
	local t = {
		...
		distortion = options.distortion or 0,

		mouse_sensitivity = 0.1,
		pitch_limit_up    = 0.3,
		pitch_limit_down  = 0.9
	}
	t.orientation = t.orientation:normalize()
...
function camera:update(w, h)
	...
	local aspect_inv = math.min(w / h, h / w)
	...
	self.projection = cpml.mat4.from_perspective(self.fov * aspect_inv, aspect, self.near, self.far)
end
